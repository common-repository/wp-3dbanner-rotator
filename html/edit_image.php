...
if( !($album = $grtr->rotator_get_album($album_id)) )
{
	die('Album id does not exists');
}
if( !($image = $grtr->get_image($image_id)) )
{
...
<?php endif;?>
	...
	do_action("rotator_admin_head-$hook_suffix");
	do_action('rotator_admin_head'); 
	?>
</head>
<body>
<div id="wpwrap">
				...
				<form id="edit_album_form" action="<?php print admin_url() ?>" method="post" enctype="multipart/form-data">
					...
					<tr>
						...
						<td>
							...
							<img src="<?php print rotator_get_album_url($album_id) . '/thumb/' . $image['thumb']; ?>" />
						</td>
					</tr>
					</table>
					<p>
