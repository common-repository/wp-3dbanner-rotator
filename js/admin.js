...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ rotator_upload_dir+"/"+file.name + '&task=rotator_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	rotator_refresh_images_table(rotator_current_album_id);
...
function rotator_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=rotator_get_albums_table', function(res)
	{
		rotator_set_data_tables();
		if (res.status == 'ok') 
		{
...
function rotator_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=rotator_get_albums_images_table&album_id='+album_id, function(res)
	{
		rotator_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'rotator_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'rotator_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'rotator_reorder_image' )
		{
			rotator_refresh_images_table(album_id);
		}
		if( task == 'rotator_reorder_album' )
			rotator_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function rotator_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', rotator_submit_bulk);
...
function rotator_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'rotator_bulk_delete_albums' || task == 'rotator_bulk_enable_albums' || task == 'rotator_bulk_disable_albums')
	{
	...
	else if( task == 'rotator_bulk_delete_images' || task == 'rotator_bulk_enable_images' || task == 'rotator_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = rotator_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: rotator_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: rotator_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: rotator_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: rotator_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : rotator_url+"/js/swfupload/js/swfupload.swf",
				upload_url: rotator_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":rotator_upload_dir, "secw":rotator_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: rotator_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.rotator_delete_album').live('click', rotator_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', rotator_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', rotator_disable);
